...
from deep_wgan import Generator, Critic, compute_gradient_penalty, calculate_fid
...
def main(config: dict):
    ...
    p = float(config["augment_pipeline"]["p"]) if "augment_pipeline" in config else 0.5
    augment = AugmentPipeline(p).cuda()
    ...
    with mlflow.start_run():
        ...
        for epoch in range(epochs):
            ...
            # Saving checkpoint
            if epoch >= warmup_epochs and min_delta < best_fid - fid:
                print("Saving best models")
                best_models["discriminator"] = C.state_dict()
                artifact_path = f"discriminator_checkpoint_epoch_{epoch}"
                mlflow.pytorch.log_model(C, artifact_path=artifact_path)

                best_models["generator"] = G.state_dict()
                artifact_path = f"generator_checkpoint_epoch_{epoch}"
                mlflow.pytorch.log_model(G, artifact_path=artifact_path)

            # Early stopping
            if min_delta < best_fid - fid:
                best_loss_epoch = epoch
                best_loss["discriminator"] = mean_C_loss
        ...
        C.load_state_dict(best_models["discriminator"])
        ...
        G.load_state_dict(best_models["generator"])
